...
class AuthController {
    async registration(req, res, next) {
        try {
            ...
            return next(Response.OK(data, "[RegSession] initialized successfully"))
        } catch (e) { 
            console.error(e)
            return next(e) 
        }
    ...
    async update(req, res, next) {
        try {
            ...
            if(logic.regexobject({username, description})) throw Response.BadRequest("Expected data not validated")
            const data = await authService.update(req.user, username, description, req.files ? req.files.pfp : null)
            return next(Response.OK(data, "Profile data successfully updated"))
        } catch (e) { 
            console.error(e)
            return next(e) 
        }
